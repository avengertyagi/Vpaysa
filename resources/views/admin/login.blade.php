...
<head>
  ...
  <style>
  .login{
    background: url("{{asset('assets/dist/images/login-new.jpeg')}}")
  }
  </style>  
</head>
...
<div class="container mx-auto h-full flex flex-1 justify-center items-center">
  <div class="w-full max-w-lg">
    <div class="leading-loose">
      <form method="post" action="{{ route('admin.showlogin') }}" class="max-w-xl m-4 p-10 bg-white rounded shadow-xl">
        @csrf
        <p class="text-gray-800 font-medium text-center text-lg font-bold">Login</p>
        <div class="">
          <label class="block text-sm text-gray-00" for="username">Email</label>
          <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="username" name="username" type="email" required="" placeholder="Email" aria-label="username">
        </div>
        <div class="mt-2">
